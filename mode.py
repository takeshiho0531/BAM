...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_img_list, support_label_list_ori, support_label_list_ori_mask, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            cv2.imwrite("/app/BAM/exp/coco/BAM/split0/resnet50/snapshot/" + 'que_gt.png', que_gt)                 # please specify the path
            cv2.imwrite("/app/BAM/exp/coco/BAM/split0/resnet50/snapshot/" + 'que_pre_f.png', que_pre_f)           # same here
            ##########
