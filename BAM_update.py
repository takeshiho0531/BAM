...
#from few_shot_seg.model.ASPP import ASPP
#from few_shot_seg.model.PSPNet import OneModel as PSPNet
...
class OneModel(nn.Module):
    def __init__(self):
        ...
        if self.shot > 1:
            ...
            if self.kshot_trans_dim == 0:
                ...
                self.kshot_rw.weight = nn.Parameter(torch.ones_like(self.kshot_rw.weight) / 5) #
            else:
    ...
    # Shared Encoder & meta_learner before merge query
    def before_init_merge_query(self, query_image_preprocessed):
        with torch.no_grad():
        ...
        return query_feat_2, query_feat_3, query_feat_4,query_feat

    # Shared Encoder & meta_learner before merge support
    def before_init_merge_support(self, support_images_batch_preprocessed, support_annotations_batch_preprocessed, weight_soft, query_feat):
        support_feat_2_list = []
        support_feat_4_list = []
        mask_list = []
        support_pro_list = []
        ...
        support_pro = torch.cat(support_pro_list, 2) ##
        support_pro = (weight_soft.permute(0,2,1,3) * support_pro).sum(2, True) ##
        concat_feat = support_pro.expand_as(query_feat) 
        return support_feat_2_list, support_feat_4_list, mask_list, concat_feat
    ...
    def forward(self, query_image_preprocessed, support_images_batch_preprocessed, support_annotations_batch_preprocessed):
        ...
        # Shared Encoder & meta_learner before merge query
        query_feat_2, query_feat_3, query_feat_4,query_feat = self.before_init_merge_query(query_image_preprocessed)
        
        # Shared Encoder & meta_learner before merge support
        support_feat_2_list, support_feat_4_list, mask_list, concat_feat = self.before_init_merge_support(support_images_batch_preprocessed, support_annotations_batch_preprocessed, weight_soft, query_feat)
        ...
        return [query_feat_2, query_feat_3, query_feat_4, mask_list, est_val, weight_soft, base_out_soft, concat_feat, corr_query_mask, meta_out_soft, merge_bg, meta_map_fg, support_feat_2_list, final_out],[concat_feat]