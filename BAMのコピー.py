...
class OneModel(nn.Module):
    ...
    # Shared Encoder query
    def shared_encoder_query(self, query_image_preprocessed):
        with torch.no_grad():
        ...
        return query_feat_2, query_feat_3, query_feat_4

    # Shared Encoder support
    def shared_encoder_support(self, support_images_batch_preprocessed, support_annotations_batch_preprocessed):
        support_feat_2_list = []
        support_feat_3_list = []
        support_feat_4_list = []
        mask_list = []
        ...
        for i in range(self.shot):
            ...
            support_feat_2_list.append(support_feat_2)
            support_feat_3_list.append(support_feat_3)
            support_feat_4_list.append(support_feat_4)
        return support_feat_2_list, support_feat_3_list, support_feat_4_list, mask_list

    # base_learner
    def base_learner(self, query_feat_4):
        base_out = self.learner_base(query_feat_4)
        base_out_soft = base_out.softmax(1)
        return base_out_soft

    # meta_learner query before merge
    def meta_learner_query(self, query_feat_3, query_feat_2):
        query_feat = torch.cat([query_feat_3, query_feat_2], 1)
        query_feat = self.down_query(query_feat)
        return query_feat

    # meta_learner support
    def meta_learner_support(self, support_feat_2_list, support_feat_3_list, mask_list, query_feat):
        support_pro_list = []
        for i in range(len(support_feat_2_list)):
            support_feat = torch.cat([support_feat_3_list[i], support_feat_2_list[i]], 1)
            support_feat = self.down_supp(support_feat)
            mask = F.interpolate(mask_list[i], size=(support_feat_3_list[i].size(2), support_feat_3_list[i].size(3)), mode='bilinear', align_corners=True)
    ...
    # similarity mask
    def meta_similarity_mask(self, mask_list, query_feat_4, support_feat_4_list, query_feat_3, weight_soft):
        corr_query_mask_list = []
        cosine_eps = 1e-7
        for i, tmp_support_feat in enumerate(support_feat_4_list):
            resize_size = tmp_support_feat.size(2)
            tmp_mask = F.interpolate(mask_list[i], size=(resize_size, resize_size), mode='bilinear', align_corners=True)

            bsize, ch_sz, sp_sz, _ = query_feat_4.size()[:]

            tmp_query = query_feat_4
            tmp_query = tmp_query.reshape(bsize, ch_sz, -1)
            tmp_query_norm = torch.norm(tmp_query, 2, 1, True)

            tmp_support_feat_4 = tmp_support_feat * tmp_mask
            tmp_support = tmp_support_feat_4
    ...
    # meta_learner merge & meta_decodeer
    def meta_learner_merge_and_decoder(self, query_feat, concat_feat, corr_query_mask):
        ...
        return meta_out_soft

    # k_shot_reweighting
    def k_shot_reweighting(self, query_feat_2, support_feat_2_list, bs):
        est_val_list = []
        que_gram = get_gram_matrix(query_feat_2)  # [bs, C, C] in (0,1)
        norm_max = torch.ones_like(que_gram).norm(dim=(1, 2))

        for i in range(len(support_feat_2_list)):
            support_gram = get_gram_matrix(support_feat_2_list[i])
        ...
        return est_val, weight_soft

    # 最後合体
    # Ensemble
    def ensemble(self, meta_out_soft, est_val, base_out_soft):
    ...
    # pm^1とf
    def merge_meta_map_fg(self, meta_out_soft, est_val):
        meta_map_fg = meta_out_soft[:, 1:, :, :]
        est_map = est_val.expand_as(meta_map_fg)  # adjustment factor
    ...
    # 最後ついに合体
    def final_concat(self, merge_bg, meta_map_fg, h, w):
    ...
    def forward(self, query_image_preprocessed, support_images_batch_preprocessed, support_annotations_batch_preprocessed):
        ...
        # Shared Encoder query
        query_feat_2, query_feat_3, query_feat_4 = self.shared_encoder_query(query_image_preprocessed)

        # Shared Encoder support
        support_feat_2_list, support_feat_3_list, support_feat_4_list, mask_list = self.shared_encoder_support(support_images_batch_preprocessed, support_annotations_batch_preprocessed)

        # K-shot reweighting
        est_val, weight_soft = self.k_shot_reweighting(query_feat_2, support_feat_2_list, bs)

        # base_learner
        base_out_soft = self.base_learner(query_feat_4)

        # meta_learner query
        query_feat = self.meta_learner_query(query_feat_3, query_feat_2)

        # meta_learner support
        concat_feat = self.meta_learner_support(support_feat_2_list, support_feat_3_list, mask_list, query_feat)

        # similarity mask
        corr_query_mask = self.meta_similarity_mask(mask_list, query_feat_4, support_feat_4_list, query_feat_3, weight_soft)

        # meta_learner merge & meta_decodeer
        meta_out_soft = self.meta_learner_merge_and_decoder(query_feat, concat_feat, corr_query_mask)

        # 最後合体
        # Ensemble
        merge_bg = self.ensemble(meta_out_soft, est_val, base_out_soft)

        # pm^1とf
        meta_map_fg = self.merge_meta_map_fg(meta_out_soft, est_val)

        # 最後ついに合体
        final_out = self.final_concat(merge_bg, meta_map_fg, h, w)
